...
export const ModuleNightLight = async (props = {}) => {
    ...
    return Widget.Button({
        ...
        onClicked: (self) => {
            ...
            if (self.attribute.enabled) Utils.execAsync('hyprsunset -t 5500').catch(print)
            else Utils.execAsync('pkill hyprsunset')
                .then(() => {
                    // disable the button until fully terminated to avoid race
                    self.sensitive = false;
                    const source = setInterval(() => {
                        Utils.execAsync('pkill -0 hyprsunset')
                            .catch(() => {
                                self.sensitive = true;
                                source.destroy();
                            });
...
export const ModuleInvertColors = async (props = {}) => {
    try {
        return Widget.Button({
            ...
            className: 'txt-small sidebar-iconbutton',
            tooltipText: getString('Muet'),
            onClicked: (self) => {
                self.attribute.enabled = !self.attribute.enabled;
                Utils.execAsync('amixer set Master toggle')
                    .then(() => self.toggleClassName('sidebar-button-active', self.attribute.enabled)) // Mettre à jour l'état du bouton après le clic
                    .catch(print);
            },
            child: MaterialIcon('volume_off', 'norm'), // Icône pour "muet"
            setup: (self) => {
                if (Utils.execAsync('amixer get Master toggle | grep off') !== '')
                {
